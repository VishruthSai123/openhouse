...
import { Home, Search, Plus, Briefcase, Users, Newspaper } from 'lucide-react';
...
const MobileBottomNav = () => {
  ...
  const allowedPaths = ['/feed', '/dashboard', '/', '/home', '/ideas', '/projects', '/jobs'];
        ...
        <div className="flex items-center justify-around h-16 px-2">
          {navItems.map((item) => {
            ...
            return (
              <button
                key={item.path}
                onClick={() => item.isAction ? setShowPostTypeSelector(true) : navigate(item.path)}
