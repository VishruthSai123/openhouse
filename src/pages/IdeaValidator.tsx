...
I'll research the market, analyze competition, and give you brutally honest feedback! 🚀`,
  ...
  const loadSession = async (sessionId: string) => {
    try {
      ...
      setConversationContext(session.conversation_context || '');
  ...
  const saveSession = async (userMsg: Message, assistantMsg: Message, hasWebContext: boolean) => {
    ...
    try {
      ...
      if (!sessionIdToUse) {
        ...
        const initialContext = `User's startup idea: ${ideaSummary || userMsg.content.slice(0, 200)}`;
        ...
        const updatedContext = `Previous discussion: Analyzed startup idea covering market viability, competition, and growth potential. Last discussed: ${userMsg.content.slice(0, 100)}`;
      ...
      <main className="flex flex-col h-[calc(100vh-56px)] sm:h-[calc(100vh-64px)] max-w-5xl mx-auto">
        ...
        <Card className="flex-1 flex flex-col shadow-xl border-border/50 rounded-none sm:rounded-lg sm:m-3">
          <CardContent className="flex-1 flex flex-col p-0 overflow-hidden">
            ...
            <ScrollArea ref={scrollRef} className="flex-1">
              <div className="space-y-2 sm:space-y-3 md:space-y-4 p-2 sm:p-3 md:p-4">
                {messages.map((message, index) => (
                  <div
                    key={index}
                    className={`flex gap-1.5 sm:gap-2 md:gap-3 ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}
                  >
                    {message.role === 'assistant' && (
                      <div className="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 rounded-full bg-gradient-to-br from-primary/20 to-primary/10 flex items-center justify-center flex-shrink-0">
                        <Brain className="w-3 h-3 sm:w-3.5 sm:h-3.5 md:w-4 md:h-4 text-primary" />
                      </div>
                    )}
                    <div
                      className={`rounded-xl sm:rounded-2xl md:rounded-3xl px-2 py-1.5 sm:px-3 sm:py-2 md:px-4 md:py-3 max-w-[90%] sm:max-w-[85%] md:max-w-[80%] shadow-sm ${
                        message.role === 'user'
                    ...
                    >
                      ...
                      <p className="text-[10px] sm:text-xs opacity-60 mt-1 sm:mt-1.5 md:mt-2">
                        {message.timestamp.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                      </p>
                    </div>
                    {message.role === 'user' && (
                      <div className="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 rounded-full bg-gradient-to-br from-primary/90 to-primary flex items-center justify-center flex-shrink-0">
                        <span className="text-[10px] sm:text-xs md:text-sm font-semibold text-primary-foreground">You</span>
                      </div>
                    )}
                  </div>
                ))}
            ...
            {/* Input Area */}
            <div className="border-t border-border/50 bg-background/95 backdrop-blur-xl p-2 sm:p-3 md:p-4 flex-shrink-0">
              <div className="flex gap-1.5 sm:gap-2">
                <Textarea
                  ...
                  className="min-h-[40px] sm:min-h-[44px] max-h-32 resize-none text-xs sm:text-sm bg-background border-border/50 focus:border-primary/50 transition-colors"
                  disabled={loading}
                />
                <Button
                  onClick={handleSend}
                  ...
                  size="icon"
                  className="h-[40px] w-[40px] sm:h-[44px] sm:w-[44px] flex-shrink-0 bg-gradient-to-br from-primary to-primary/80 hover:from-primary/90 hover:to-primary/70 transition-all shadow-lg hover:shadow-primary/25"
                >
